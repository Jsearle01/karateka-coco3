-- gate1_live.lua — load princess_gate1.bin (throne composite) and let it run
-- LIVE (throttled 60fps) for the Jay visual gate (AC-4). No snapshot, no exit.
...
local function load(p) local f=io.open(p,"rb"); if not f then return end
  ...
  return ex end
local st="wait"
_G._g1l=emu.add_machine_frame_notifier(function()
