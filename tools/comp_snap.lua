...
_G._c=emu.add_machine_frame_notifier(function()
  ...
  if st=="wait" and f>=300 and cpu.state["PC"].value>=0x8000 then
    local ex=load(BIN); if ex then cpu.state["PC"].value=ex end st="run"; f0=f; return end
  if st=="run" and f-f0>=120 then pcall(function() scr:snapshot() end); manager.machine:exit() end
end)
