...
cmd.exe /c "cd /d C:\karateka-capture && C:\mame\mame.exe coco3 \
    ...
    > build/debugtrace_mame.log 2>&1 || true

echo ""
echo "--- Trace result ---"
if [ -f "$CAPTURE_DIR/tools/instrtrace.log" ]; then
    cp "$CAPTURE_DIR/tools/instrtrace.log" build/
    echo "instrtrace.log: $(wc -l < build/instrtrace.log) lines"
    echo ""
    echo "--- First 120 lines ---"
    head -120 build/instrtrace.log
    echo ""
    echo "--- Last 60 lines ---"
    tail -60 build/instrtrace.log
else
    echo "No trace log produced — check build/debugtrace_mame.log:"
    tail -20 build/debugtrace_mame.log 2>/dev/null || true
fi
echo ""
echo "=== Trace done ==="
